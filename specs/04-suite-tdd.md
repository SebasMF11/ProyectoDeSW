...
- **Herramientas Seleccionadas:** **Node.js Test Runner** (`node:test`) y **Node Assert** (`node:assert`) para pruebas unitarias, con cobertura nativa de V8 mediante `--experimental-test-coverage`.
...
1. **Ficha Técnica del Entorno de Pruebas:** Node.js, `node:test`, `node:assert`, cobertura V8 y mocks de Supabase.
2. **Matriz de Casos de Prueba:** Lista detallada de cada test con su propósito y resultado.
3. **Evidencia de Ejecución en Consola:** Salida textual formateada de `npm run test:coverage`.
